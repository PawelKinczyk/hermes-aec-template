...
## Required invocation templates (Hermes rollout — Stage 2)

This section is mandatory for every Claude Code invocation from Hermes
(Stage 2 of the `hermes-rollout` roadmap). Flags verified on this machine
2026-07-16 (`claude --version` = 2.1.210, `claude --help`) — **`--max-turns`
does not exist in this CLI version** (absent from `claude --help`), even
though the JSON result still contains `num_turns` and may return
`subtype: error_max_turns` (internal limit, no external flag control).
Cost/time control goes exclusively through `--max-budget-usd` and shell
`timeout`.

Use the full binary path (Stage 0): `claude`
— do not rely on `claude` from the interactive session PATH.

### Base template (mandatory pattern)

```bash
cd <repo_path> && timeout <limit_s> claude \
  -p "<narrow, complete prompt>" \
  --allowedTools "<list per task type>" \
  --permission-mode acceptEdits \
  --max-budget-usd <budget> \
  --output-format json
```

### Profiles per task type

| Task type | allowedTools | timeout (s) | max-budget-usd |
|---|---|---|---|
| Small fix / hotfix | `Read,Edit,Bash(git *)` | 600 | 5.50 |
| Issue implementation | `Read,Edit,Bash(git *),Bash(npm test),Bash(pytest *)` | 1800 | 7.00 |
| Post-review fixes (resume) | `Read,Edit,Bash(git *)` | 900 | 6.00 |
| Repo analysis (read-only) | `Read,Bash(git diff *),Bash(git log *)` | 600 | 5.30 |

(Budgets raised by $4 across the board 2026-07-17 per user decision - existing
OpenRouter-side spend caps remain the hard backstop.)

### Mandatory rules

1. One issue = one invocation. The prompt contains: issue number, plan,
   acceptance criteria, no-push-without-consent rule (`git commit` on a new
   branch, no `git push`).
2. Claude Code cannot see the Telegram conversation — the entire context
   must be in the prompt (Claude Code has no access to Hermes chat history).
3. After every invocation — INCLUDING ones that ended in an error
   (error_budget, error_max_turns, timeout): parse the result JSON and
   append an entry to `~/.hermes/state/claude-code-costs.jsonl`:
   ```json
   {"ts":"ISO8601","task":"short description","repo":"name","cost_usd":0.0,"turns":0,"session_id":"..."}
   ```
   (`cost_usd` from `total_cost_usd`, `turns` from `num_turns` in the JSON
   response), then report the result and cost on Telegram. The cost of a
   failed invocation was still incurred and must be logged.
4. Continuing work: `claude -p --resume
   "<session_id>" "..."` — always from the same repo directory.
5. `--dangerously-skip-permissions` is forbidden.
6. If `is_error=true`, `subtype` starts with `error_`, or `timeout`/
   `max-budget-usd` was exceeded: do not retry automatically; report to the
   user with the error content.
